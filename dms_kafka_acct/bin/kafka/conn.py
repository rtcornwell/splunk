...
class BrokerConnection(object):
    ...
    Keyword Arguments:
        ...
        sasl_plain_password (str): password for sasl PLAIN authentication.
            Required if sasl_mechanism is PLAIN.
        sasl_dms_ak (str): Access Key for sasl DMS authentication.
            Required if sasl_mechanism is PLAIN.
        sasl_dms_secret (str): secret for sasl DMS authentication.
            Required if sasl_mechanism is DMS.
    ...
    DEFAULT_CONFIG = {
        ...
        'sasl_dms_ak': None,
        'sasl_dms_sk': None,
        'sasl_dms_projectid': None,
        'sasl_kerberos_service_name': 'kafka',
        'sasl_kerberos_domain_name': None,
        'sasl_oauth_token_provider': None
    }
    SECURITY_PROTOCOLS = ('PLAINTEXT', 'SSL', 'SASL_PLAINTEXT', 'SASL_SSL')
    SASL_MECHANISMS = ('PLAIN', 'GSSAPI', 'OAUTHBEARER', 'DMS')
    ...
    def __init__(self, host, port, afi, **configs):
        ...
        if self.config['security_protocol'] in ('SASL_PLAINTEXT', 'SASL_SSL'):
            ...
            if self.config['sasl_mechanism'] == 'DMS':
                assert self.config['sasl_dms_ak'] is not None, 'sasl_dms_ak required for DMS sasl'
                assert self.config['sasl_dms_sk'] is not None, 'sasl_dms_sk required for DMS sasl'
                assert self.config['sasl_dms_projectid'] is not None, 'sasl_dms_projectid required for DMS sasl'
    ...
    def _handle_sasl_handshake_response(self, future, response):
        ...
        elif self.config['sasl_mechanism'] == 'DMS':
            return self._try_authenticate_dms(future)
    ...
    def _try_authenticate_dms(self, future):

        data = b''
        # Send PLAIN credentials per RFC-4616
        msg = bytes('\0'.join([self.config['sasl_dms_ak'],
                               self.config['sasl_dms_sk'],
                               self.config['sasl_dms_projectid']]).encode('utf-8'))
        size = Int32.encode(len(msg))
        try:
            with self._lock:
                if not self._can_send_recv():
        ...
        log.info('%s: Authenticated as %s via DMS', self, self.config['sasl_dms_ak'])
        return future.success(True)
