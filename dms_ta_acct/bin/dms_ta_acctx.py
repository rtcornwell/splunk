...
def get_token(UserName, UserPass, Domain):
    ...
    resp = http_post(url, Header, body, verify=False, cert=None, proxies=None, cookies=None)
...
def ackmessage(handlers, TokenID, ProjectID, QueueID, ConsumerGroupID):
    ...
    body = json.dumps(body, indent= 4, encoding="ütf-8", sort_keys=True, separators=(',', ':'))
    Header = dict()
    Header.setdefault("Content-type", "application/json")
    Header.setdefault("X-Auth-Token", TokenID)
    resp = http_post(ackurl, Header, body, verify=False, cert=None, proxies=None, cookies=None )
...
def processqueue(TokenID, ProjectID, QueueID, ConsumerGroupID, max_msgs):
    ...
    resp = http_get(DMSurl, header, verify=False, cert=None, proxies=None, cookies=None)
    if resp.status_code == requests.codes.ok:
        if len(resp.content) > 2 : 
            ...
            for Event in Events:
                ...
                messages = json.loads(Event['message']['body'])
                ...
                send_event(json.dumps(message), edate)
                i += 1

            fini_stream()
            # Acknowledge the batch of messages just processed.
            success = ackmessage(MsgHandlers, TokenID, ProjectID, QueueID, ConsumerGroupID)
            return True
        else:
            return False
    else:
...
def run():
    ...
    # ConsumerGroupID = "g-6c34eed5-4957-430d-85d3-50f5e7460602"
